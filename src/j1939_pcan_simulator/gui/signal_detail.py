"""Right panel for editing all parameters of the selected signal.

Min/max values are shown as paired raw and physical fields with live cross
updates. Sine and ramp parameters appear only when their simulation modes are
selected.
"""
...
from j1939_pcan_simulator.gui.theme import repolish
from j1939_pcan_simulator.gui.waveform import WaveformWidget
from j1939_pcan_simulator.protocol.identifier import PgnCategory, parse_can_id
...
class _RawPhysPair(QWidget):
    """Paired raw and physical editors that update each other live."""
    ...
    def set_scale_offset(self, scale: float, offset: float) -> None:
        self._scale = scale
        self._offset = offset
        # Reflect the current raw value into the physical editor.
        self._suppress = True
        try:
...
class SignalDetail(QWidget):
    ...
    # Emitted when signal data is edited.
    ...
    def set_signal(self, message: Optional[Message], signal: Optional[Signal]) -> None:
        ...
        try:
            ...
            # Apply scale/offset to paired editors before loading values.
    ...
    def refresh_after_external_edit(self) -> None:
        """Refresh the form after external signal changes, such as inline edits."""
    ...
    def _on_value_changed(self, *_args) -> None:
        ...
        # Sawtooth uses the minimum sentinel as the special "auto" value.
